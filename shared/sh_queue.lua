...
local function playerConnect(name, setKickReason, deferrals)
    ...
    if not ids then
        ...
        CancelEvent()
        Queue:DebugPrint("Drop " .. name .. ", n'a pu récupérer aucune de leurs pièces d'identité")
        return
    end
...
if Config.DisableHardCap then
    Queue:DebugPrint("^1 [connectqueue] Désactivation de hardcap ^7")
...
commands.removeq = function(args)
    ...
    Queue:DebugPrint("RETIRÉ " .. tostring(name) .. " DE LA FILE D'ATTENTE")
end
...
commands.printq = function()
    ...
    for pos, data in ipairs(Queue:GetQueueList()) do
        Queue:DebugPrint(pos .. ": [src: " .. data.source .. "] " .. data.name .. "[" .. data.ids[1] .. "] | Priorité: " .. (tostring(data.priority and data.priority or false)) .. " | Last Msg: " .. (data.source ~= "debug" and GetPlayerLastMsg(data.source) or "debug") .. " | Timeout: " .. data.timeout .. " | Queue Time: " .. data.queuetime() .. " Secondes")
    end
end

commands.addc = function()
    Queue:AddToConnecting({"debug"})
    Queue:DebugPrint("AJOUT DE LA FILE D'ATTENTE DE DÉBOGAGE DE CONNEXION")
end
...
commands.removec = function(args)
    ...
    Queue:DebugPrint("SUPPRIMÉ " .. tostring(name) .. " DE LA LISTE DE CONNEXION")
end

commands.printc = function()
    Queue:DebugPrint("LISTE DE CONNEXION ACTUELLE")

    for pos, data in ipairs(Queue:GetConnectingList()) do
        Queue:DebugPrint(pos .. ": [src: " .. data.source .. "] " .. data.name .. "[" .. data.ids[1] .. "] | Priorité: " .. (tostring(data.priority and data.priority or false)) .. " | Last Msg: " .. (data.source ~= "debug" and GetPlayerLastMsg(data.source) or "debug") .. " | Timeout: " .. data.timeout)
    end
end
...
commands.printp = function()
    Queue:DebugPrint("LISTE DES PRIORITÉS ACTUELLES")
...
commands.printcount = function()
    Queue:DebugPrint("Nombre de joueurs: " .. Queue:GetPlayerCount())
end

commands.printtp = function()
    Queue:DebugPrint("LISTE DES PRIORITÉS ACTUELLES EN MATIÈRE D'INTÉRIM")

    for k, data in pairs(Queue:GetTempPriorityList()) do
        Queue:DebugPrint(k .. ": Puissance: " .. tostring(data.power) .. " | EndTime: " .. tostring(data.endTime) .. " | CurTime: " .. tostring(os_time()))
    end
end
...
commands.removetp = function(args)
    ...
    Queue:DebugPrint("RETIRÉ " .. args[1] .. " DE LA LISTE DE PRIORITÉ DES INTÉRIMAIRES")
end
...
commands.setpos = function(args)
    ...
    Queue:DebugPrint("RÉGLER " .. data.name .. "'s A LA POSITION DE LA FILE D'ATTENTE SUR: " .. args[2])
end
